...
func testnetCmd(mbm module.BasicManager, genBalIterator banktypes.GenesisBalancesIterator) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "testnet name-of-the-authority-key",
		Short: "Initialize files for a emapp testnet",
...
Example:
	emd testnet name-of-the-authority-key --v 4 --keyring-backend test --keyaccounts path-to-the-test-keyfile  --output-dir ./output --starting-ip-address 192.168.10.2
	`,
...
func InitTestnet(clientCtx client.Context, cmd *cobra.Command, nodeConfig *tmconfig.Config, mbm module.BasicManager, genBalIterator banktypes.GenesisBalancesIterator, outputDir, chainID, minGasPrices, nodeDirPrefix, nodeDaemonHome, startingIPAddress, keyringBackend, algoStr string, numValidators int, authorityKey sdk.AccAddress, keybaseAccountPath string) error {
	...
	for i := 0; i < numValidators; i++ {
		...
		accStakingTokens := sdk.TokensFromConsensusPower(6000, sdk.DefaultPowerReduction)
		coins := sdk.Coins{
		...
		valTokens := sdk.TokensFromConsensusPower(6000, sdk.DefaultPowerReduction)
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			sdk.ValAddress(addr),
			valPubKeys[i],
			sdk.NewCoin(stakingToken, valTokens),
			stakingtypes.NewDescription(nodeDirName, "", "", "", ""),
			stakingtypes.NewCommissionRates(sdk.NewDecWithPrec(15, 2), sdk.NewDecWithPrec(100, 2), sdk.NewDecWithPrec(100, 2)),
			sdk.OneInt(),
		)
		if err != nil {
			return err
